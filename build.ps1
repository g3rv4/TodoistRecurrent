...
docker run --rm -v "$($basePath):/var/src" mcr.microsoft.com/dotnet/core/sdk:3.1.302-alpine3.12 ash -c "dotnet publish -c Release /var/src/TodoistRecurrent.csproj -o /var/src/bin/build && chown -R $($uid):$($gid) /var/src"
